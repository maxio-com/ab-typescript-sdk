...
| `id` | `bigint \| undefined` | Optional | The unique identifier for the webhook (unique across all of Chargify). This is not changed on a retry/replay of the same webhook, so it may be used to avoid duplicate action for the same event. |
...
| `successful` | `boolean \| undefined` | Optional | “A boolean flag describing whether the webhook was accepted by the webhook endpoint for the most recent attempt. (Acceptance is defined by receiving a “200 OK” HTTP response within a reasonable timeframe, e.g., 15 seconds.)” |
...
```ts
import { Webhook } from '@maxio-com/advanced-billing-sdk';
