...
import {
  CleanupScopeEnum,
  cleanupScopeEnumSchema,
} from '../models/cleanupScopeEnum';
import {
...
export class SitesController extends BaseController {
   ...
   *
   *
   * @param cleanupScope  `all`: Will clear all products, customers, and related subscriptions
   *                                          from the site.  `customers`: Will clear only customers and related
   *                                          subscriptions (leaving the products untouched) for the site.  Revenue
   *                                          will also be reset to 0. Use in query `cleanup_scope=all`.
   * @return Response from the API call
   */
  async clearSite(
    ...
    const mapped = req.prepareArgs({
      cleanupScope: [cleanupScope, optional(cleanupScopeEnumSchema)],
    });
