...
} from '../errors/subscriptionsInvoicesJson422Error';
import {
  ConsolidatedInvoice,
...
import { DirectionEnum, directionEnumSchema } from '../models/directionEnum';
import { Invoice, invoiceSchema } from '../models/invoice';
import {
  InvoiceDateFieldEnum,
  invoiceDateFieldEnumSchema,
} from '../models/invoiceDateFieldEnum';
import {
  InvoiceEventTypeEnum,
  invoiceEventTypeEnumSchema,
} from '../models/invoiceEventTypeEnum';
import {
  InvoiceResponse,
...
import {
  ...
  invoiceSortFieldEnumSchema,
} from '../models/invoiceSortFieldEnum';
import {
  IssueInvoiceRequest,
...
export class InvoicesController extends BaseController {
   ...
   *                                                       specified.
   * @param status                 The current status of the invoice.  Allowed Values: draft,
   ...
   *                                                       consolidated invoices for. This will return a paginated list
   *                                                       of consolidated invoices for the specified group.
   ...
   *                                                       instead of start_date. Allowed to be used only along with
   *                                                       date_field set to created_at or updated_at.
   ...
   *                                                       specify timezone in query - otherwise your site's time zone
   *                                                       will be used. If provided, this parameter will be used
   *                                                       instead of end_date. Allowed to be used only along with
   *                                                       date_field set to created_at or updated_at.
   ...
   * @param number                 Allows fetching invoices with matching invoice number based
   *                                                       on provided values. Use in query `number=1234,1235`.
   * @param productIds             Allows fetching invoices with matching line items product
   *                                                       ids based on provided values. Use in query `product_ids=23,
   *                                                       34`.
   * @param sort                   Allows specification of the order of the returned list. Use
  ...
  async listInvoices(
    ...
    req.query('customer_ids', mapped.customerIds as string);
   ...
   * @param eventTypes                 Filter results by event_type. Supply a comma
   *                                                             separated list of event types (listed above). Use in
   *                                                             query: `event_types=void_invoice,void_remainder`.
   * @return Response from the API call
   */
  async listInvoiceEvents(
    ...
    const mapped = req.prepareArgs({
      ...
      eventTypes: [eventTypes, optional(array(invoiceEventTypeEnumSchema))],
    });
  ...
  async createInvoice(
    ...
    req.throwOn(422, SubscriptionsInvoicesJson422Error, 'Unprocessable Entity (WebDAV)');
