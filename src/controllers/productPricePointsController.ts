...
} from '../errors/productPricePointsCurrencyPricesJson422Error';
import {
  BasicDateFieldEnum,
...
import {
  ...
  listProductsPricePointsIncludeEnumSchema,
} from '../models/listProductsPricePointsIncludeEnum';
import {
  PricePointTypeEnum,
...
export class ProductPricePointsController extends BaseController {
   ...
   *                                                Default value is 10. The maximum allowed values is 200; any
   *                                                per_page value over 200 will be changed to 200.
   * @param currencyPrices  When fetching a product's price points, if you have defined
   ...
   *                                                price data in the response. If the product price point is set to
   *                                                use_site_exchange_rate: true, it will return pricing based on the
   *                                                current exchange rate. If the flag is set to false, it will return
  ...
  async createProductCurrencyPrices(
    ...
    req.throwOn(422, ProductPricePointsCurrencyPricesJson422Error, 'Unprocessable Entity (WebDAV)');
  ...
  async listAllProductPricePoints(
    ...
    const mapped = req.prepareArgs({
      ...
      filterType: [filterType, optional(array(pricePointTypeEnumSchema))],
      include: [include, optional(listProductsPricePointsIncludeEnumSchema)],
      page: [page, optional(number())],
      perPage: [perPage, optional(number())],
    });
