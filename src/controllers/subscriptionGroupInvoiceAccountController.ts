...
import {
  ...
  listSubscriptionGroupPrepaymentDateFieldEnumSchema,
} from '../models/listSubscriptionGroupPrepaymentDateFieldEnum';
import {
...
export class SubscriptionGroupInvoiceAccountController extends BaseController {
   ...
   *                                                                           time zone on the date specified. Use in
   ...
   *                                                                           displayed. The page parameter specifies
   *                                                                           a page number of results to fetch. You
   *                                                                           can start navigating through the pages
   *                                                                           to consume the results. You do this by
   *                                                                           passing in a page parameter. Retrieve
   *                                                                           the next page by adding ?page=2 to the
   *                                                                           query string. If there are no results to
   *                                                                           return, then an empty result set will be
   *                                                                           returned. Use in query `page=1`.
   * @param perPage            This parameter indicates how many
   *                                                                           records to fetch in each request.
   *                                                                           Default value is 20. The maximum allowed
   *                                                                           values is 200; any per_page value over
   *                                                                           200 will be changed to 200. Use in query
   *                                                                           `per_page=200`.
   * @return Response from the API call
   */
  async listPrepaymentsForSubscriptionGroup(
    ...
    const mapped = req.prepareArgs({
      ...
      filterDateField: [
        filterDateField,
        optional(listSubscriptionGroupPrepaymentDateFieldEnumSchema),
      ],
