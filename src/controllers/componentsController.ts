...
import {
  ...
  listAllComponentPricePointsDirectionSchema,
} from '../models/containers/listAllComponentPricePointsDirection';
import {
...
export class ComponentsController extends BaseController {
   ...
   *                                                                       to filter the date_field. Returns price
   *                                                                       points with a timestamp up to and including
   *                                                                       11:59:59PM in your site’s time zone on the
   *                                                                       date specified.
   ...
   * @param filterStartDatetime    The start date and time (format YYYY-MM-DD
   *                                                                       HH:MM:SS) with which to filter the
   *                                                                       date_field. Returns price points with a
   *                                                                       timestamp at or after exact time provided in
   *                                                                       query. You can specify timezone in query -
   *                                                                       otherwise your site's time zone will be used.
   *                                                                       If provided, this parameter will be used
   *                                                                       instead of start_date.
   * @param filterType             Allows fetching price points with matching
   *                                                                       type. Use in query: `filter[type]=custom,
   *                                                                       catalog`.
   * @param direction              Controls the order in which results are
   *                                                                       returned. Use in query `direction=asc`.
   * @param filterIds              Allows fetching price points with matching
   *                                                                       id based on provided values. Use in query:
   *                                                                       `filter[ids]=1,2,3`.
  ...
  async listAllComponentPricePoints(
    ...
    const mapped = req.prepareArgs({
      ...
      direction: [
        direction,
        optional(listAllComponentPricePointsDirectionSchema),
      ],
      filterIds: [filterIds, optional(array(number()))],
