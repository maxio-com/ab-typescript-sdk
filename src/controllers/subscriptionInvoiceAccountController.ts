...
} from '../errors/subscriptionsPrepaymentsRefundsJson400Error';
import {
  SubscriptionsPrepaymentsRefundsJson422Error,
} from '../errors/subscriptionsPrepaymentsRefundsJson422Error';
import {
  AccountBalances,
...
import {
  ...
  subscriptionsPrepaymentsJsonResponseSchema,
} from '../models/subscriptionsPrepaymentsJsonResponse';
...
export class SubscriptionInvoiceAccountController extends BaseController {
   ...
   *                                                 created_at - Time when prepayment was created. application_at -
   *                                                 Time when prepayment was applied to invoice. Use in query
   *                                                 `filter[date_field]=created_at`.
   * @param filterStartDate    The start date (format YYYY-MM-DD) with which to filter the
   *                                                 date_field. Returns prepayments with a timestamp at or after
   ...
   *                                                 date_field. Returns prepayments with a timestamp up to and
  ...
  async refundPrepayment(
    ...
    req.throwOn(400, SubscriptionsPrepaymentsRefundsJson400Error, 'Bad Request');
    req.throwOn(404, ApiError, 'Not Found');
    req.throwOn(422, SubscriptionsPrepaymentsRefundsJson422Error, 'Unprocessable Entity (WebDAV)');
